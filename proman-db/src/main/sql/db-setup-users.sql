...
-- # Step 1 - USERS table
DROP TABLE IF EXISTS proman.USERS;
...
CREATE TABLE proman.USER_AUTH_TOKENS(
	...
	ORIGIN_IPADDRESS VARCHAR(20) NOT NULL,
	EXPIRES_AT TIMESTAMP NOT NULL,
	LOGIN_AT TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
	LOGOUT_AT TIMESTAMP,
	LOGOUT_ACTION SMALLSERIAL,
	CREATED_BY VARCHAR(100) NOT NULL,
	CREATED_AT TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
COMMENT ON COLUMN proman.USER_AUTH_TOKENS.REFRESH_TOKEN IS 'Generated authentication refresh token (long-lived) for the logged-in user';
COMMENT ON COLUMN proman.USER_AUTH_TOKENS.CLIENT_ID IS 'Reference to the client specific application that the user has logged-in from';
COMMENT ON COLUMN proman.USER_AUTH_TOKENS.ORIGIN_IPADDRESS IS 'Reference to the system ip address (v6 supported) that the user has logged-in from';
...
ALTER TABLE proman.USER_AUTH_TOKENS ADD CONSTRAINT UK_USER_AUTH_TOKENS_REFRESH_TOKEN UNIQUE(REFRESH_TOKEN);
