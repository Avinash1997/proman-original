...
5. Go to `proman-db` module and build the project using maven command `mvn install -Psetup` to setup the database schema called proman, users with necessary, all tables and default groups, roles and permissions data.
    ```
    Note: Predefined groups, roles and permissions can be modified by changing the SQL scripts, which are present in proman-db/src/main/sql folder.
    ```

6. In order to setup sample admin user you can run command `mvn install -Ptest-data`.
